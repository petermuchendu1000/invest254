...
export const GATEWAY_SCHEMAS: Record<string, GatewaySchema> = {
  ...
  payhero: {
    ...
    blurb: "M-Pesa STK / bank / paybill routing via PayHero. Authenticates with a Basic auth token; STK pushes go through a payment channel.",
    fields: [
      { key: "env", label: "Environment", kind: "select", secret: false, required: true, default: "sandbox", options: ENV_SANDBOX_PROD },
      { key: "auth_token", label: "Basic auth token", kind: "secret", secret: true, required: true, placeholder: "Base64 token from PayHero → API Keys", help: "The 'Basic Authorization token' shown when you create an API key. Stored encrypted." },
      { key: "channel_id", label: "Payment channel ID", kind: "text", secret: false, required: true, placeholder: "e.g. 1487", help: "The channel STK pushes are routed through (GET /payment_channels)." },
      { key: "account_id", label: "Account ID", kind: "text", secret: false, required: false, placeholder: "Optional PayHero account id" },
      { key: "callback_url", label: "Callback URL", kind: "url", secret: false, required: false, placeholder: "https://your-api/…/payhero/callback" },
      { key: "base_url", label: "API base URL", kind: "url", secret: false, required: false, default: "https://backend.payhero.co.ke/api/v2" },
    ],
  },
};
