...
test("admin lists users and reads the overview", async () => {
  ...
  try {
    ...
    const list = await req(api, "GET", "/api/v1/admin/users", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" });
    assert.equal(list.status, 200);
    const body = await json(list);
    assert.ok(Array.isArray(body.items) && body.items.length >= 2);
    const ov = await json(await req(api, "GET", "/api/v1/admin/overview", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }));
...
test("admin suspend is applied + audited; login still works (deposits) but the account is limited", async () => {
  ...
  try {
    ...
    const sus = await req(api, "POST", `/api/v1/admin/users/${uid}/suspend`, { token: "admin-9:admin:00000000-0000-0000-0000-000000000001", body: { reason: "abuse" } });
...
test("admin cannot suspend another admin; a superadmin can", async () => {
  ...
  try {
    ...
    const denied = await req(api, "POST", `/api/v1/admin/users/${uid}/suspend`, { token: "admin-2:admin:00000000-0000-0000-0000-000000000001" });
...
test("admin can make/clear a marketer as brand default; a player token is forbidden (routes wired + gated)", async () => {
  ...
  try {
    ...
    assert.equal((await req(api, "POST", `/api/v1/admin/marketers/${mk}/make-default`, { token: "root-1:admin:00000000-0000-0000-0000-000000000001" })).status, 200);
    assert.equal((await req(api, "POST", `/api/v1/admin/marketers/${mk}/clear-default`, { token: "root-1:admin:00000000-0000-0000-0000-000000000001" })).status, 200);
  } finally { await api.close(); }
});
...
test("admin manual balance adjustment credits the wallet, requires a reason, and is audited", async () => {
  ...
  try {
    ...
    const credit = await req(api, "POST", `/api/v1/admin/wallets/${uid}/adjust`, { token: "fin-1:admin:00000000-0000-0000-0000-000000000001", body: { amountCents: 25_000, reason: "manual credit" } });
    ...
    const debit = await req(api, "POST", `/api/v1/admin/wallets/${uid}/adjust`, { token: "fin-1:admin:00000000-0000-0000-0000-000000000001", body: { amountCents: 5_000, direction: "debit", reason: "clawback" } });
...
test("admin deposits monitor lists deposits and the reconcile read returns a summary + stale list", async () => {
  ...
  try {
    ...
    const list = await json(await req(api, "GET", "/api/v1/admin/deposits", { token: "fin-1:admin:00000000-0000-0000-0000-000000000001" }));
    ...
    const rec = await json(await req(api, "GET", "/api/v1/admin/deposits/reconcile?staleMinutes=0", { token: "fin-1:admin:00000000-0000-0000-0000-000000000001" }));
...
test("admin reports: per-day & per-user JSON, CSV export, and the date-range filter (J4)", async () => {
  ...
  try {
    ...
    // Per-user JSON.
    const users = (await json(await req(api, "GET", "/api/v1/admin/reports/users", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }))).items as any[];
    ...
    const daily = (await json(await req(api, "GET", "/api/v1/admin/reports/daily", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }))).items as any[];
    ...
    const csvRes = await req(api, "GET", "/api/v1/admin/reports/daily?format=csv", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" });
    ...
    const filtered = (await json(await req(api, "GET", "/api/v1/admin/reports/daily?from=2030-01-01", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }))).items as any[];
    assert.ok(!filtered.some((r) => r.date === "2026-06-10"));

    // Malformed date -> 400; player token -> 403.
    assert.equal((await req(api, "GET", "/api/v1/admin/reports/daily?from=2026/06/10", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" })).status, 400);
...
test("admin reports/day (calendar): comprehensive single-day stats; date-validated; role-gated", async () => {
  ...
  try {
    ...
    const res = await req(api, "GET", `/api/v1/admin/reports/day?date=${today}`, { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" });
    ...
    assert.equal((await req(api, "GET", "/api/v1/admin/reports/day", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" })).status, 200);
    // Malformed date -> 400; player token -> 403.
    assert.equal((await req(api, "GET", "/api/v1/admin/reports/day?date=2026/06/10", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" })).status, 400);
...
test("admin withdrawals queue carries full player context (balance + lifetime deposits/withdrawals)", async () => {
  ...
  try {
    ...
    const items = (await json(await req(api, "GET", "/api/v1/admin/withdrawals", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }))).items as any[];
...
test("admin withdrawals kill switch: role-gated toggle that blocks new withdrawals when off", async () => {
  ...
  try {
    ...
    assert.equal((await json(await req(api, "GET", "/api/v1/admin/withdrawals-enabled", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }))).enabled, true);
    ...
    assert.equal((await req(api, "PUT", "/api/v1/admin/withdrawals-enabled", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001", body: {} })).status, 400);

    // Admin disables -> a player withdrawal request is refused (403 WITHDRAWALS_DISABLED).
    const off = await req(api, "PUT", "/api/v1/admin/withdrawals-enabled", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001", body: { enabled: false } });
    ...
    await req(api, "PUT", "/api/v1/admin/withdrawals-enabled", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001", body: { enabled: true } });
...
test("marketer expenses (0068): admin logs an expense; the marketer sees it (transparency); role-gated", async () => {
  ...
  try {
    ...
    const e1 = await req(api, "POST", "/api/v1/admin/affiliate/expenses", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001", body: { marketerUserId: uid, category: "tiktok_promo", amountCents: 5_000, note: "Boost" } });
    assert.equal(e1.status, 200);
    assert.equal((await json(e1)).category, "tiktok_promo");
    await req(api, "POST", "/api/v1/admin/affiliate/expenses", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001", body: { marketerUserId: uid, category: "advance", amountCents: 20_000 } });

    // Admin can list them (newest-first, with a total).
    const adminList = await json(await req(api, "GET", `/api/v1/admin/affiliate/expenses?marketerUserId=${uid}`, { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }));
    ...
    assert.equal((await req(api, "POST", "/api/v1/admin/affiliate/expenses", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001", body: { marketerUserId: uid, amountCents: 100 } })).status, 400);
    assert.equal((await req(api, "POST", "/api/v1/admin/affiliate/expenses", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001", body: { marketerUserId: uid, category: "other", amountCents: -5 } })).status, 400);
  } finally { await api.close(); }
});
...
test("marketer expenses (BUGLOG #23): totalCents is the FULL sum, never the limit-capped page sum", async () => {
  ...
  try {
    ...
    for (const c of [7_000, 11_000, 13_000]) {
      assert.equal((await req(api, "POST", "/api/v1/admin/affiliate/expenses", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001", body: { marketerUserId: uid, category: "data_bundles", amountCents: c } })).status, 200);
    }
    ...
    const adminList = await json(await req(api, "GET", `/api/v1/admin/affiliate/expenses?marketerUserId=${uid}&limit=2`, { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }));
...
test("J5 game config: admin reads; only superadmin edits; validates; audited", async () => {
  const api = await startTestApi();
  try {
    const cfg = await json(await req(api, "GET", "/api/v1/admin/game-config", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }));
...
test("M-Pesa config: admin reads masked; only superadmin edits; secrets write-only; audited", async () => {
  ...
  try {
    ...
    assert.equal((await req(api, "PATCH", "/api/v1/admin/mpesa-config", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001", body: { shortcode: "174379" } })).status, 403);
...
test("user role: admin promotes player<->marketer; superadmin full power; validates; no self-action; audited", async () => {
  ...
  try {
    ...
    assert.equal((await req(api, "POST", `/api/v1/admin/users/${uid}/role`, { token: "admin-1:admin:00000000-0000-0000-0000-000000000001", body: { role: "admin" } })).status, 403);
    const adminUp = await req(api, "POST", `/api/v1/admin/users/${uid}/role`, { token: "admin-1:admin:00000000-0000-0000-0000-000000000001", body: { role: "marketer" } });
...
test("J5 RTP monitor: target derived from house_edge, rolling windows, no alert on empty data", async () => {
  const api = await startTestApi();
  try {
    const rtp = await json(await req(api, "GET", "/api/v1/admin/rtp", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }));
...
test("J5 seed rotation: superadmin-only, future-day-only, bumps version, listed + audited", async () => {
  ...
  try {
    ...
    assert.equal((await req(api, "POST", "/api/v1/admin/seeds/rotate", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001", body: { tradeDate: "2999-01-01" } })).status, 403);
    ...
    const seeds = await json(await req(api, "GET", "/api/v1/admin/seeds", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }));
...
test("J6 affiliate payout queue: admin lists requests and approves (audited)", async () => {
  ...
  try {
    ...
    const queue = await json(await req(api, "GET", "/api/v1/admin/affiliate/payouts?status=requested", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }));
    ...
    const appr = await req(api, "POST", `/api/v1/admin/affiliate/payouts/${payout.payoutId}/approve`, { token: "admin-9:admin:00000000-0000-0000-0000-000000000001" });
...
test("admin user activity: merges deposits, withdrawals and bets newest-first with kind filter + paging", async () => {
  ...
  try {
    ...
    const all = await json(await req(api, "GET", `/api/v1/admin/users/${uid}/activity`, { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }));
    ...
    const betsOnly = await json(await req(api, "GET", `/api/v1/admin/users/${uid}/activity?kind=bet`, { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }));
    ...
    for (let guard = 0; guard < 10; guard++) {
      ...
      const page = await json(await req(api, "GET", url, { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" }));
    ...
    assert.equal((await req(api, "GET", `/api/v1/admin/users/${uid}/activity?kind=bogus`, { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" })).status, 400);
...
test("fly restart: restarts engine, skips the serving (self) machine + stopped ones, aggregates result", async () => {
  ...
  try {
    ...
    assert.equal((await req(api, "POST", "/api/v1/admin/fly/restart", { token: "a:admin:00000000-0000-0000-0000-000000000001" })).status, 403);
...
test("admin transactions: unified deposits+withdrawals feed with kind filter, search and validation", async () => {
  ...
  try {
    ...
    const all = await json(await req(api, "GET", "/api/v1/admin/transactions", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001" }));
    ...
    // kind filter
    const deps = await json(await req(api, "GET", "/api/v1/admin/transactions?kind=deposit", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001" }));
    assert.ok(deps.items.every((t: any) => t.kind === "deposit"), "kind=deposit filters");

    // status filter (only the completed deposit)
    const success = await json(await req(api, "GET", "/api/v1/admin/transactions?status=success", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001" }));
    ...
    // search by username
    const searched = await json(await req(api, "GET", "/api/v1/admin/transactions?q=txapi", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001" }));
    assert.ok(searched.items.length >= 1 && searched.items.every((t: any) => t.username.includes("txapi")), "q search");

    // invalid kind -> 400
    const bad = await req(api, "GET", "/api/v1/admin/transactions?kind=bogus", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001" });
...
test("admin users: numeric filters (minDepositsCents, minBalanceCents) narrow the list", async () => {
  ...
  try {
    ...
    const all = await json(await req(api, "GET", "/api/v1/admin/users", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001" }));
    ...
    const rich = await json(await req(api, "GET", "/api/v1/admin/users?minDepositsCents=1", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001" }));
    ...
    const bal = await json(await req(api, "GET", "/api/v1/admin/users?minBalanceCents=50000", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001" }));
    ...
    const bad = await req(api, "GET", "/api/v1/admin/users?minDepositsCents=abc&minBets=-3", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001" });
...
test("admin reset-balance: restores a user's real wallet to their last funded amount", async () => {
  ...
  try {
    ...
    // requires a reason
    assert.equal((await req(api, "POST", `/api/v1/admin/users/${uid}/reset-balance`, { token: "adm-1:admin:00000000-0000-0000-0000-000000000001", body: {} })).status, 400);

    const res = await json(await req(api, "POST", `/api/v1/admin/users/${uid}/reset-balance`, { token: "adm-1:admin:00000000-0000-0000-0000-000000000001", body: { reason: "settlement bug" } }));
    assert.equal(res.lastFundedCents, 40_000);
    assert.equal(res.newBalanceCents, 40_000);

    const detail = await json(await req(api, "GET", `/api/v1/admin/users/${uid}`, { token: "adm-1:admin:00000000-0000-0000-0000-000000000001" }));
...
test("admin bulk: mass suspend / notify / reset-balance with per-user partial results", async () => {
  ...
  try {
    ...
    // bulk suspend both
    const susp = await json(await req(api, "POST", "/api/v1/admin/users/bulk", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001", body: { action: "suspend", userIds: [a, b], reason: "audit" } }));
    assert.equal(susp.okCount, 2);
    assert.equal(susp.failCount, 0);
    for (const uid of [a, b]) {
      const d = await json(await req(api, "GET", `/api/v1/admin/users/${uid}`, { token: "adm-1:admin:00000000-0000-0000-0000-000000000001" }));
      assert.equal(d.status, "suspended");
    }

    // bulk notify both
    const notify = await json(await req(api, "POST", "/api/v1/admin/users/bulk", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001", body: { action: "notify", userIds: [a, b], title: "Maintenance", level: "warning" } }));
    assert.equal(notify.okCount, 2);
    for (const uid of [a, b]) {
      const list = await json(await req(api, "GET", `/api/v1/admin/users/${uid}/notifications`, { token: "adm-1:admin:00000000-0000-0000-0000-000000000001" }));
      assert.ok(list.items.some((n: any) => n.title === "Maintenance"));
    }

    // bulk reset-balance both -> back to 30k
    const reset = await json(await req(api, "POST", "/api/v1/admin/users/bulk", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001", body: { action: "reset-balance", userIds: [a, b], reason: "recover" } }));
    ...
    const mixed = await json(await req(api, "POST", "/api/v1/admin/users/bulk", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001", body: { action: "suspend", userIds: [a, "adm-1"], reason: "audit" } }));
    assert.equal(mixed.okCount, 1);
    assert.equal(mixed.failCount, 1);
    assert.ok(mixed.results.find((r: any) => r.userId === "adm-1" && !r.ok));

    // validation
    assert.equal((await req(api, "POST", "/api/v1/admin/users/bulk", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001", body: { action: "suspend", userIds: [] } })).status, 400);
    assert.equal((await req(api, "POST", "/api/v1/admin/users/bulk", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001", body: { action: "bogus", userIds: [a] } })).status, 400);
    assert.equal((await req(api, "POST", "/api/v1/admin/users/bulk", { token: "adm-1:admin:00000000-0000-0000-0000-000000000001", body: { action: "reset-balance", userIds: [a] } })).status, 400); // reason required
...
test("system logs (/admin/logs) are owner-only: a site admin is refused, the system owner allowed (BUGLOG #33)", async () => {
  const api = await startTestApi();
  try {
    assert.equal((await req(api, "GET", "/api/v1/admin/logs", { token: "admin-1:admin:00000000-0000-0000-0000-000000000001" })).status, 403, "admin refused");
