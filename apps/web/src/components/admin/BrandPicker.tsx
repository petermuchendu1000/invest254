...
export function BrandPicker() {
  ...
  return (
    <div className="flex w-full max-w-3xl flex-col gap-4">
      <div>
        <h1 className="text-xl font-semibold tracking-tight">Choose a brand to open</h1>
          ...
          {rows.map((s) => (
            <li key={s.siteId} className="flex items-center justify-between gap-3 p-3">
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-muted">{s.primaryDomain ?? s.slug}{s.status !== 'active' ? ` · ${s.status}` : ''}</p>
              </div>
