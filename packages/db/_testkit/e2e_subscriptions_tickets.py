...
def main():
    ...
    print("\n== Lifecycle auto-advance: Trial -> Past Due -> Grace -> Suspended ==")
    ...
    check("trial expired -> past_due", q1(cur,"select status from platform_subscriptions where platform_id=%s",[pl])[0]=="past_due")
    q1(cur,"update platform_subscriptions set current_period_end=now()-interval '8 days' where platform_id=%s",[pl])
    q1(cur,"select fn_subscription_auto_advance()")
    ...
    evs=q1(cur,"select count(*) from subscription_events where platform_id=%s and reason in ('trial_expired','grace','suspended')",[pl])[0]
    check("lifecycle transitions all logged", evs==3, f"events={evs}")
    q1(cur,"select fn_subscription_record_payment(%s,%s,%s,%s,%s)",[ACTOR,SYS,pl,100000,30])
    r=q1(cur,"select status, current_period_end>now() from platform_subscriptions where platform_id=%s",[pl])
    check("payment reactivates + extends period", r==("active",True), str(r))
