...
- 🟡 Thread `ctx.siteId` into every service/repository call. **Done:** history reads
  (`/wallet/ledger`, `/positions`, `/positions/:id`, `/transactions`) and the wallet + payments
  money core (`/wallet`, `/deposits`, `/withdrawals`) now run under `requireSite` and pass
  `ctx.siteId` through PaymentService + PaymentRepository into the site-aware
  `fn_create_deposit`/`fn_create_withdrawal` RPCs and site-filtered wallet reads (null → default
  brand, so single-tenant is unchanged). Tested (brand-B deposit stamped to B, invisible to the
  default brand; cross-brand history isolation).
  **Blocked (needs Task B / F first):** affiliate + admin. Their RPCs (`fn_affiliate_enroll`,
  `fn_accrue_affiliate_commissions`, all `fn_admin_*`) are NOT site-aware — the tables carry
  `site_id` but the functions neither accept nor filter on it. Threading `ctx.siteId` into those
  handlers now would be a no-op at best and risk cross-brand commission leakage at worst, so it is
  deliberately deferred until the affiliate accrual is site-grouped (Task B) and admin roles/RLS
  land (Tasks F, H).
- ✅ M-Pesa callback routes accept the `/s/<slug>` prefix and resolve the site (`app.payments.ts`):
  ...
  slug → `ctx.siteId` before the handler; unprefixed routes stay for the default brand. Tested.
- **Done when:** every list/mutation is implicitly filtered to the caller's site; the brand route
  returns correct per-host branding. (Brand route + auth scoping + middleware + callback prefix +
  history/wallet/payments threading ✅; affiliate + admin threading blocked on Tasks B/F/H.)
